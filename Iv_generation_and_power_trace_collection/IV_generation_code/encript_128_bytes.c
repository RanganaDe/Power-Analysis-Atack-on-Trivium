...
int main(int argc, char ** argv)
{
    ...
    u8 out[64];  
    u8 key[10] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09};
    u8 iv[10];
	...
	char iv_set[25];
   	...
   	FILE *fp_in;    // input plain text
   	FILE *fp_out;   // output file
   	FILE *fp_in_ivs;  // input ivs

   	//open regarded files
   	// ./trivium.out <plain.txt> <output.txt>
   	if(argc == 3){
	...
	fgets(buffer, sizeof(buffer), fp_in);
   	...
   	while (fgets(iv_set, sizeof(iv_set), fp_in_ivs) != NULL){
