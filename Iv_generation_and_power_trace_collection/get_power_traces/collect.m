...
outputfile='generated_ciphertext.txt';
...
if (pwrcol==1)
    ...
    if isempty(interfaceObj)
		...
		% You can find this identifier in Instrument Control Toolbox
        interfaceObj = visa('TEK', 'USB0::0x0699::0x039E::C010440::0::INSTR');
    else
        fclose(interfaceObj);
...
for i=start:last
	...
	%send the IV sample to the device and check whether the cryptosystem prints back correctly
    ...
    if (pwrcol==1)
		...
		%for the first power trace acquire the the wave form of the trigger as well
    ...
    while((verifyen==1 && (strcmp(cout,cverify)==0)))
            flushinput(s);
            flushoutput(s);
            fprintf(s,'%s',pin);
